...
execute as @s[scores={deeper_dark.var=0}] run tellraw @a [{"text":"["},{"text":"deeper_dark","color":"#007A8A"},{"text":"] "},{"text":"Something went wrong. Please try rejoining the singleplayer world/restarting the server. If the issue persists, report the bug on Planet Minecraft. [err_dim_not_exist]","color":"red"}]
...
playsound minecraft:block.portal.ambient ambient @s ~ ~ ~ 1 2 1
playsound minecraft:block.sculk_shrieker.shriek ambient @s ~ ~ ~ 1 0 1
playsound minecraft:block.respawn_anchor.set_spawn ambient @s ~ ~ ~ 1 0 1
playsound minecraft:block.enchantment_table.use ambient @s ~ ~ ~ 1 2 1
